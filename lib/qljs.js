...
var render = function (ctx, Component) {
    return Array.isArray(ctx) ? (ctx.map(function (ctx) { return (react_1.default.createElement(Component, __assign({}, ctx, { transact: function (query) { return transact(ctx, query); }, render: render }))); })) : (react_1.default.createElement(Component, __assign({}, ctx, { transact: function (query) { return transact(ctx, query); }, render: render })));
};
var RootComponent;
var element;
var state;
...
exports.parseQueryIntoProps = parseQueryIntoProps;
...
function performRemoteQuery(query) {
    if (remoteHandler && Array.isArray(query) && query.length > 0) {
        var _a = __read(query, 1), term = _a[0];
        var _b = __read(compressTerm(term), 2), tag = _b[0], params = _b[1];
        remoteHandler(tag, params).then(function (results) {
...
function refresh(_a) {
    ...
    if (RootComponent !== undefined) {
        var perfRQ = skipRemote
            ? function (v) { return v; }
            : function (query) {
                performRemoteQuery(parseQueryRemote(query));
                return query;
            };
        var props_1 = parseQueryIntoProps(perfRQ(getQuery(RootComponent)));
        react_dom_1.default.render(react_1.default.createElement(RootComponent, __assign({}, props_1, { transact: function (query) { return transact(props_1, query); }, render: render })), element);
    }
}
...
function loopRootQuery(queryTerm, __env) {
    if (__env) {
        var __parentEnv = __env.__parentEnv;
        var newEnv = __assign({}, (__parentEnv ? mapDelta(__parentEnv, __env) : __env));
...
exports.loopRootQuery = loopRootQuery;
...
function makeRootQuery(__env, query) {
    return query.map(function (queryTerm) {
        return loopRootQuery(queryTerm, __env.__parentEnv);
    });
}
exports.makeRootQuery = makeRootQuery;
