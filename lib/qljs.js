...
/*!***********************!*\
  !*** ./src/index.tsx ***!
  \***********************/
/*! exports provided: multimethod, read, mutate, remote, sync, render, component, getQuery, clearRegistry, parseQueryIntoMap, parseChildrenRemote, zip, mapDelta, loopRootQuery, makeRootQuery, parseChildren, transact, componentToQuery, unfoldQueryTerm, unfoldQuery, mount */
...
const render = (ctx, Component) => Array.isArray(ctx) ? (ctx.map(ctx => (react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(Component, Object.assign({}, ctx, { transact: query => transact(ctx, query) }))))) : (react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(Component, Object.assign({}, ctx, { transact: query => transact(ctx, query) })));
let Component;
let element;
let state;
let remoteHandler;
...
function makeCtx(res, [k, v]) {
    return Object.assign(Object.assign({}, res), { [k]: v });
}
function parseQueryIntoMap(__query, __env = {}, _state = state) {
    ...
    return Object.assign({ __env,
        __query, key: typeof key === "string" ? key : "unique" }, ctx);
}
...
function refresh({ skipRemote } = { skipRemote: true }) {
    if (Component !== undefined) {
        ...
        react_dom__WEBPACK_IMPORTED_MODULE_1___default.a.render(react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(Component, Object.assign({}, ctx, { transact: query => transact(ctx, query) })), element);
    }
}
...
function mount({ state: _st, remoteHandler: _rh, component, element: _el }) {
    state = _st;
    remoteHandler = _rh;
    Component = component;
    element = _el;
