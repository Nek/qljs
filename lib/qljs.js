...
/*!**********************!*\
  !*** ./src/index.ts ***!
  \**********************/
/*! exports provided: multimethod, query, getQuery, clearRegistry, parseQueryIntoMap, parseChildrenRemote, zip, mapDelta, loopRootQuery, makeRootQuery, parseChildren, transact, createInstance, componentToQuery, unfoldQueryTerm, unfoldQuery, mount */
...
function unfoldQueryTerm(foldedTerm) {
    let foldedTerms;
    const [tag, maybeAtts] = foldedTerm;
    let res;
    if (maybeAtts && (Object.getPrototypeOf(maybeAtts) === Object.prototype)) {
    ...
    const terms = foldedTerms.map(componentToQuery).reduce((res, arr) => ([...res, ...arr]), []);
