...
/*!**********************!*\
  !*** ./src/index.ts ***!
  \**********************/
/*! exports provided: multimethod, query, getQuery, clearRegistry, parseQueryIntoMap, parseChildrenRemote, mapDelta, loopRootQuery, makeRootQuery, parseChildren, transact, createInstance, mount, _mount */
...
function parseQueryIntoMap(query, env, _state = state, _parsers = parsers) {
    const queryName = query.map(_utils__WEBPACK_IMPORTED_MODULE_2__["first"]);
    const queryResult = parseQuery(query, env);
    const atts = Object(_utils__WEBPACK_IMPORTED_MODULE_2__["zip"])(queryName, queryResult).reduce((res, [k, v]) => (Object.assign({}, res, { [k]: v })), {});
    return Object.assign({ env,
        query }, atts);
}
function parseQueryRemote(query) {
    return query.reduce((acc, item) => {
        if (parsers.remote[Object(_utils__WEBPACK_IMPORTED_MODULE_2__["first"])(item)]) {
...
function performRemoteQuery(query) {
    if (Array.isArray(query) && remoteHandler) {
        remoteHandler(query, results => {
            Object(_utils__WEBPACK_IMPORTED_MODULE_2__["zip"])(query, results).map(([k, v]) => parseQueryTermSync(k, v, {}));
            refresh(false);
        });
    }
}
...
function createInstance(Component, atts) {
    ...
    return react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(Component, Object.assign({}, atts, { env,
        query, key: env.id ? env.id : '_' }));
}
...
function mount({ state: _st, parsers: _parsers, remoteHandler: _rh }) {
    state = _st;
    parsers = _parsers;
    remoteHandler = _rh;
...
function _mount({ component, element: _el, state: _st, remoteHandler: _rh, parsers: _parsers, }) {
    Component = component;
    element = _el;
    state = _st;
    remoteHandler = _rh;
...
/*!**********************!*\
  !*** ./src/utils.ts ***!
  \**********************/
/*! exports provided: flatten, extractJson, zip, first */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "flatten", function() { return flatten; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "extractJson", function() { return extractJson; });
