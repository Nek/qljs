...
var render = function (ctx, Component) {
    if (Array.isArray(ctx)) {
        return ctx.map(function (ctx) { return (React.createElement(Component, __assign({}, ctx, { transact: function (query) { return transact(ctx, query); }, render: render }))); });
    }
    else if (isProps(ctx)) {
        return (React.createElement(Component, __assign({}, ctx, { transact: function (query) { return transact(ctx, query); }, render: render })));
    }
    else {
        return null;
    }
...
exports.component = function (query, target) {
    var fullQuery = query.map(tagToTerm).map(addParamsAndNormalize);
...
function parseQueryRemote(query) {
    return query.reduce(function (acc, term) {
...
function loopRootQuery(queryTerm, __env) {
    if (__env) {
        var newProps = __assign(__assign({}, (__env.__parentProps ? mapDelta(__env.__parentProps, __env) : __env)), { __parentProps: undefined, __queryKey: undefined });
        return loopRootQuery([__env.__queryKey, newProps, queryTerm], __env.__parentProps);
    }
    else {
        return queryTerm;
    }
...
function parseChildren(term, __env, _state) {
    ...
    var newProps = __assign(__assign({}, __env), { __parentProps: __assign(__assign({}, __env), { __queryKey: term[0] }) });
...
function transact(_a, query, _state) {
    ...
    if (_state === void 0) { _state = state; }
    var fullQuery = query.map(tagToTerm).map(addParamsAndNormalize);
    var rootQuery = __spread(fullQuery, componentQuery).map(function (queryTerm) {
